...
f = open('TGFs_Analysed_durations_ver3.txt','w' )
...
#trying to mask the values
    ...
    with fits.open(evt_file) as hdul:
        for quarter in range(1,5):
            ...
            debug(f"{row[0]}_{quarter}_debug_ver3.txt", f"{names_of_regions[0]}:{regions[0]}, {names_of_regions[1]}:{regions[1]}, {names_of_regions[2]}:{regions[2]}, {names_of_regions[3]}:{regions[3]}\n")
            # print(regions)
    ...
    for k_index in range(len(hists_by_reg)):
        ...
        hdu.writeto(
            f"tables/{binning}/{row[0][:-6]}_{row[0][-5:]}_V1.0_{binning}_3_CoincSigmaTable_V2_{names_of_regions[k_index]}_ver3.fits",
            overwrite=True,
        )
        print(names_of_regions[mark], Coinc_Tables[mark]*10000/Analysed_Durations[mark])
        # print('writing: ', coinc_sigma_table)
        print('writing data to: ', f"tables/{binning}/{row[0][:-6]}_{row[0][-5:]}_V1.0_{binning}_3_CoincSigmaTable_V2_{names_of_regions[k_index]}_ver3.fits")
