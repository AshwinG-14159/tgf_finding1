...
f = open('TGFs_Analysed_durations_polar.txt','w' )
...
skip_till = ['20230518_T05_106T01_9000005642_level2_41295', 'T05_106T01_9000005642', 'cjain', '4U 1626-67', '248.0697', '-67.46095', '9537.51990873', '2023-05-18T16:02:50', '2023-05-18T20:48:43']
skipped = 0
...
#trying to mask the values
    ...
    try:
        with fits.open(evt_file) as hdul:
            for quarter in range(1,5):
                ...
                # debug(f"{row[0]}_{quarter}_debug_ver4.txt", f"{names_of_regions[0]}:{regions[0]}, {names_of_regions[1]}:{regions[1]}, {names_of_regions[2]}:{regions[2]}, {names_of_regions[3]}:{regions[3]}\n")
                # print(regions)
    ...
    Analysed_Durations+=hist.shape[1]
    f.write(f'Region, Analysed duration: {hist.shape[1]};\n')
    ...
    hdu.writeto(
        f"tables/{binning}/{row[0][:-6]}_{row[0][-5:]}_V1.0_{binning}_3_CoincSigmaTable_V2_polar.fits",
        overwrite=True,
    )
    print( Coinc_Tables*1000/Analysed_Durations)
    # print('writing: ', coinc_sigma_table)
    print('writing data to: ', f"tables/{binning}/{row[0][:-6]}_{row[0][-5:]}_V1.0_{binning}_3_CoincSigmaTable_V2_polar.fits")
